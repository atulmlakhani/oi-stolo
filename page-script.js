; (() => {
  ...
  const start = async () => {
    ...
    const apply = async () => {
      ...
      const cols = document.querySelectorAll('.tabulator-col-group');
      cols.forEach(col => {
        const title = col.getAttribute('aria-title');
        const match = title?.match(/\((\d+)\)/);
        const strike = match ? Number(match[1]) : null;
        strikePrice = strike;
      });

      // Send rowData to local POST endpoint (non-blocking, with short timeout)
      (async () => {
        try {
          if (Array.isArray(rowData) && rowData.length) {
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), 1000);
            await fetch('https://ind-api.visasontime.com/send-stolo-data', {
              method: 'POST',
          ...
          console.debug('oi-stolo: failed to POST rowData', err);
        }
      })();
    ...
    // Utility: convert a DOM row into a small JSON object containing only
    // the requested fields: `call.total_oi`, `put.total_oi`, and `time`.
    const getRowData = (row) => {
      if (!row) return null;
      const wanted = ['call.total_oi', 'put.total_oi', 'time'];
      const out = {};

      wanted.forEach(f => {
        const txt = cellText(row, f, 'last');
        const n = num(txt);
        out[f] = txt;
        if (Number.isFinite(n)) out[`${f}_num`] = n;
        if (f === 'time') out['time_norm'] = normTime(txt);
      });
